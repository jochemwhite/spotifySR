function handleMessage(message: string) {
    ...
    let rawTagsComponent = null; // everything before the first space ake user infomaion
    ...
    if (message[idx] === "@") {
      ...
      let endIdx = message.indexOf(" "); //first space it can find
      rawTagsComponent = message.slice(1, endIdx);
      idx = endIdx + 1; // Should now point to source colon (:).
    }
  
    // console.log("rawTagsComponent" + rawTagsComponent); logs the user information BUT not the message
    ...
    rawCommandComponent = message.slice(idx, endIdx);
